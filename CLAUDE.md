...
**규모 등급** (떡볶이 기준선 비교):
- HOT (30%+) → 초록 pill 배지 — 이미 많이 찾는 중
- 주목 (10~29%) → 노란 pill 배지 — 신상품 치고 검색 잡히는 수준
- 초기 (3~9%) → 노란 pill 배지 — 검색 시작 단계
- 3% 미만 → 배지 없음 — 본문에서 "SNS에서 빠르게 퍼지는 중" 등 맥락 설명

**주간 변화율**: 최근 7일 vs 이전 7일 평균 비교 (% 숫자로 제공, ▲/▼ 라벨 없음)
- 배지 옆에 `HOT -44%` 식으로 표기
- 해석은 뉴스레터 본문에서 맥락에 맞게 (예: "출시 직후 피크 대비 소강 중이지만 여전히 핫해요")

### 3단계: 키워드 분류 (웹 검색)
각 키워드를 편의점 입점 여부로 분류:
- 편의점 입점/신상 확인 → **편의점 트렌드** (상태값 배지 HOT/주목/초기 + 주간 변화율%)
...
- 트렌드 상태값(HOT/주목/초기 + 주간 변화율%)은 반드시 네이버 DataLab + 웹 검색으로 판단. 추측 금지.
